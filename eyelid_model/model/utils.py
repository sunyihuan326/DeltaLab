...
def load_data(dataset='eyelid', train_mode='train'):
    if dataset == 'eyelid':
        ...
        file_queue = tf.train.string_input_producer([record_file], shuffle=True, capacity=500, num_epochs=num_epochs)
        ...
        image_dim = 64
        image = tf.decode_raw(features['image_raw'], tf.float32) * (1. / 255)
