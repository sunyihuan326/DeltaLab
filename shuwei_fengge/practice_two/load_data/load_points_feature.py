...
def main():
    ...
    for i, file in enumerate(files):
        ...
        data_X[i, :] = np.array(points).reshape(1, -1)
        data_Y[i, :] = scio.loadmat(label_dir + '/' + file.replace('Point', 'Label'))['Label']
        print('read_{}_data------->loading----->end'.format(file))

    scio.savemat('face_1_channel_XY_Points72', {"X": data_X, "Y": data_Y})


if __name__ == '__main__':
    tt = scio.loadmat('face_1_channel_XY_Points72.mat')
    print(tt['X'].shape)
    # main()
